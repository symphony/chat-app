...
// = local data =
const onlineUsers: UserDB = {
  test: { id: "abcdefg", username: "welcome-bot" },
};
...
const scrub = (s: string) => {
  const r1: [RegExp, string] = [
    /n+[\s_\.\-]?[i1l|]+[\s_\.\-]?[gq]+[_\.\-\s]*[gq69]+[e3a4i\s]*[ra4s]*s?/gi,
    "****",
  ];
  const r2: [RegExp, string] = [
    /(f|ph)?[\s_\.\-]?[a4]+.?[gq69]+[_\.\-\s]*[gq69]+[o0\s]+[t7]*s?/gi,
    "****",
  ];
...
const emitUserlist = (
  ...
  if (id)
    return socket.to(id).emit("userlist", { users: Object.values(users) });
